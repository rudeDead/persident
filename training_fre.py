"""
FreeSolv Molecular Property Prediction with Multi-Baseline Enhanced Integrated Gradients (MB-EIG)

This training pipeline implements the complete MB-EIG system for hydration free energy prediction.
Adapted from ESOL implementation with FreeSolv-specific modifications:
- Different dataset structure (SAMPL.csv)
- Property: Hydration Free Energy (kcal/mol)
- Interpretation: Negative values = stable in water (favorable)
- Fragment patterns adapted for solvation energy
"""
...
log_file = LOGS_DIR / 'freesolv_training.log'
logging.basicConfig(
    level=logging.INFO,
...
logger.info("FreeSolv Training with MB-EIG - Logger initialized")

# ============================================================================
# COPY MOLECULAR GRAPH AND MODEL COMPONENTS FROM ESOL (Same architecture)
# ============================================================================
...
class FreeSolvDataset(Dataset):
    """FreeSolv Dataset for Hydration Free Energy"""
    
    def __init__(self, data: pd.DataFrame, max_atoms: int = 100, target_column: str = 'expt'):
        self.data = data
        self.max_atoms = max_atoms
        ...
        print(f"Loading {len(self.data)} molecules from FreeSolv dataset...")
...
# Graphormer Model Components (identical to ESOL)
...
# ============================================================================
# MB-EIG IMPLEMENTATION FOR FREESOLV (Adapted patterns for hydration energy)
# ============================================================================
...
class MBEIGExplainer:
    ...
    def _generate_human_explanation(self, fragments: Dict[str, float], prediction: float) -> str:
        ...
        explanation = f"HYDRATION FREE ENERGY PREDICTION: {prediction:.3f} kcal/mol\n\n"
        
        if stable_score > unstable_score:
            explanation += "🔵 STABILIZING groups dominate this molecule!\n"
            key_groups = [f for f in stabilizing if abs(fragments.get(f, 0)) > 0.01]
            if key_groups:
                explanation += f"Key groups: {', '.join(key_groups)}\n"
            explanation += "These polar groups form strong interactions with water, stabilizing the molecule.\n"
            explanation += "Prediction: NEGATIVE ΔG (favorable hydration)"
        else:
            explanation += "🔴 DESTABILIZING groups dominate this molecule!\n"
            key_groups = [f for f in destabilizing if abs(fragments.get(f, 0)) > 0.01]
            if key_groups:
                explanation += f"Key groups: {', '.join(key_groups)}\n"
            explanation += "These hydrophobic groups disrupt water structure, destabilizing the molecule.\n"
            explanation += "Prediction: POSITIVE ΔG (unfavorable hydration)"
        
        return explanation

def save_molecule_visualization(smiles: str, attributions: List[float], 
                               property_name: str, prediction: float, save_path: Path):
    """Render molecule with attribution-based coloring for FreeSolv"""
    try:
        ...
        # Color scheme: Blue (stabilizing/negative ΔG) to Red (destabilizing/positive ΔG)
        atom_colors = {}
        atom_radii = {}
...
def train_model_with_mb_eig(model, train_loader, val_loader, val_dataset, 
    ...
    logger.info("Starting FreeSolv training with MB-EIG explanations...")
    ...
    for epoch in range(num_epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save({
                ...
                'val_loss': val_loss,
                'val_r2': val_r2,
            }, MODELS_DIR / 'best_graphormer_freesolv.pth')
        ...
        if epoch % 5 == 0:
            ...
            for idx in sample_indices:
                ...
                if explanation:
                    ...
                    vis_path = EXPLAIN_DIR / f"epoch_{epoch+1}_{smiles.replace('/', '_')}_freesolv_molecule.png"
                    save_molecule_visualization(
                        smiles, explanation['mb_eig_attributions'], 
                        'FreeSolv Hydration Energy', explanation['prediction'], vis_path
                    )
            
            # Save explanation batch
            if explanation_results:
                batch_path = EXPLAIN_DIR / f'epoch_{epoch+1}_freesolv_explanations.json'
    ...
    visualize_training_results(train_losses, val_losses, val_r2_scores, 
                             RESULTS_DIR / 'freesolv_training_history.png')
    
    return train_losses, val_losses, val_r2_scores

def main():
    """Main training function for FreeSolv with MB-EIG"""
    logger.info("="*80)
    logger.info("FREESOLV MOLECULAR PROPERTY PREDICTION WITH MB-EIG")
    logger.info("="*80)
    ...
    NUM_EPOCHS = 100
    LEARNING_RATE = 1e-4
    TARGET_COLUMN = 'expt'  # Experimental hydration free energy
    
    try:
        # Load dataset
        logger.info("Loading FreeSolv dataset...")
        data = pd.read_csv(CSV_PATH)
        logger.info(f"Dataset loaded: {data.shape}")
        logger.info(f"Columns: {list(data.columns)}")
        
        dataset = FreeSolvDataset(data, MAX_ATOMS, TARGET_COLUMN)
        ...
        checkpoint = torch.load(MODELS_DIR / 'best_graphormer_freesolv.pth', map_location=device)
        ...
        with open(RESULTS_DIR / 'freesolv_final_results.json', 'w') as f:
            json.dump(final_results, f, indent=2)
        
        logger.info("Training completed successfully!")
        logger.info(f"Model saved to: {MODELS_DIR / 'best_graphormer_freesolv.pth'}")
