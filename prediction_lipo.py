"""
Lipophilicity Single Molecule Prediction with MB-EIG Explanations

This script provides complete explanations for individual molecules using the trained Lipophilicity model.
Usage: python lipo_prediction.py --smiles "CCC(C)CO"
       python lipo_prediction.py --smiles "CCO" --output custom_output_dir
...
OUTPUT_DIR = Path('../xai_lipo')

def setup_logging(output_dir: Path):
    """Setup logging for prediction script"""
    log_file = output_dir / 'lipo_prediction.log'
    logging.basicConfig(
        level=logging.INFO,
...
# ============================================================================
# COPY ALL MODEL COMPONENTS FROM LIPO.PY (Same architecture)
# ============================================================================
...
# ============================================================================
# MB-EIG EXPLAINER FOR LIPOPHILICITY
# ============================================================================
...
class MBEIGExplainer:
    ...
    def _format_fragments_for_output(self, fragment_scores: Dict[str, float], 
        ...
        for fragment_name, score in fragment_scores.items():
            if abs(score) > 1e-6:  # Only include non-zero fragments
                ...
                if score > 0:
                    interpretation = "Increases lipophilicity (oil-loving)"
                else:
                    interpretation = "Decreases lipophilicity (water-loving)"
    ...
    def _generate_human_explanation(self, fragments: Dict[str, float], prediction: float) -> str:
        ...
        explanation = "To see if this molecule prefers oil or water, we ran a 'tug-of-war' between its parts.\n\n"
        
        if oil_score > water_score:
            explanation += "🔵 The BLUE parts (oil-loving groups) dominate this molecule!\n"
            key_groups = [f for f in oil_loving if abs(fragments.get(f, 0)) > 0.01]
            if key_groups:
                explanation += f"Key groups: {', '.join(key_groups)}\n"
            explanation += "These hydrophobic groups are on the 'Oil Team', strongly increasing its logP score.\n\n"
            explanation += f"Prediction: {prediction:.3f} logP - INCREASED lipophilicity (more oil-loving)"
        else:
            explanation += "🔴 The RED parts (water-loving groups) dominate this molecule!\n"
            key_groups = [f for f in water_loving if abs(fragments.get(f, 0)) > 0.01]
            if key_groups:
                explanation += f"Key groups: {', '.join(key_groups)}\n"
            explanation += "These hydrophilic groups are on the 'Water Team', decreasing its logP score.\n\n"
            explanation += f"Prediction: {prediction:.3f} logP - DECREASED lipophilicity (more water-loving)"
...
def save_molecule_visualization(smiles: str, attributions: List[float], 
    ...
    try:
        ...
        # Color scheme: Blue (oil-loving) to Red (water-loving)
        atom_colors = {}
        atom_radii = {}
...
def create_attribution_plots(explanation_data: Dict[str, Any], output_dir: Path, logger):
    ...
    try:
        ...
        fig.suptitle(f"MB-EIG Attribution Analysis for Lipophilicity\nSMILES: {smiles}", fontsize=14)
        ...
        axes[0, 1].set_title('MB-EIG Attributions (Blue=Oil-loving, Red=Water-loving)')
...
def save_text_explanation(explanation_data: Dict[str, Any], output_dir: Path, logger):
    ...
    try:
        ...
        with open(text_path, 'w', encoding='utf-8') as f:
            ...
            f.write("Generated by MB-EIG Explainer for Lipophilicity\n")
...
def predict_single_molecule(smiles: str, output_dir: Path = None, model_path: Path = None):
    ...
    logger.info("=" * 80)
    logger.info("LIPOPHILICITY SINGLE MOLECULE PREDICTION WITH MB-EIG")
    logger.info("=" * 80)
    ...
    try:
        ...
        if model_path is None:
            model_path = MODELS_DIR / 'best_graphormer_lipo.pth'
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Lipophilicity Single Molecule Prediction with MB-EIG")
    parser.add_argument('--smiles', type=str, required=True,
                       help='SMILES string of the molecule to analyze')
    parser.add_argument('--output', type=str, default=None,
                       help='Output directory (default: ../xai_lipo)')
    parser.add_argument('--model', type=str, default=None,
                       help='Path to trained model (default: ../models/best_graphormer_lipo.pth)')
    ...
    if result:
        ...
        print(f"Predicted Lipophilicity: {result['predicted_value']:.3f} {result['unit']}")
