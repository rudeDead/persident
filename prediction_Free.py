"""
FreeSolv Single Molecule Prediction with MB-EIG Explanations

This script provides complete explanations for individual molecules using the trained FreeSolv model.
Usage: python prediction.py --smiles "CCO"
       python prediction.py --smiles "c1ccccc1" --output custom_output_dir

Property: Hydration Free Energy (kcal/mol)
Interpretation: Negative values = favorable hydration (stable in water)
                Positive values = unfavorable hydration (prefers non-polar environment)

Output includes:
- Prediction value and confidence
- MB-EIG attributions with dual baseline analysis
- Fragment analysis adapted for solvation energy
- Molecule visualization with attribution coloring
- Human-readable explanation for hydration thermodynamics
"""
...
def setup_logging(output_dir: Path):
    ...
    log_file = output_dir / 'freesolv_prediction.log'
    logging.basicConfig(
        level=logging.INFO,
...
# ============================================================================
# MB-EIG EXPLAINER FOR FREESOLV (Same as training.py)
# ============================================================================

class MBEIGExplainer:
    """Multi-Baseline Enhanced Integrated Gradients for FreeSolv"""
    ...
    def __init__(self, model, device='cpu', integration_steps: int = 50):
        ...
        # SMARTS patterns for hydration free energy-relevant fragments
        self.fragment_patterns = {
            # Stabilizing in water (negative ΔG - favorable)
            'H-Bond Donor: -OH': '[OH]',
            'H-Bond Donor: -NH2': '[NH2]', 
            'H-Bond Donor: -NH': '[NH1]',
            'H-Bond Acceptor: C=O': '[CX3]=[OX1]',
            'H-Bond Acceptor: -O-': '[OX2;H0]',
            'H-Bond Acceptor: -N-': '[NX3;H0]',
            'Charged: Carboxylate': '[CX3](=O)[O-]',
            'Charged: Ammonium': '[NX4+]',
            
            # Destabilizing in water (positive ΔG - unfavorable)  
            'Hydrophobic: Aromatic': 'c1ccccc1',
            'Hydrophobic: Long Alkyl': '[CH2][CH2][CH2][CH2]',
            'Hydrophobic: Branched Alkyl': '[CX4;H0]([CX4])([CX4])[CX4]',
            'Hydrophobic: Halogens': ['[F]', '[Cl]', '[Br]', '[I]']
        }
    ...
    def compute_mb_eig(self, smiles: str) -> Optional[Dict[str, Any]]:
        """Compute Multi-Baseline Enhanced Integrated Gradients for FreeSolv"""
        try:
    ...
    def _format_fragments_for_output(self, fragment_scores: Dict[str, float], 
        ...
        for fragment_name, score in fragment_scores.items():
            if abs(score) > 1e-6:  # Only include non-zero fragments
                ...
                # Generate interpretation for FreeSolv
                if "H-Bond" in fragment_name or "Charged" in fragment_name:
                    interpretation = "Stabilizes hydration (negative ΔG contribution)"
                elif "Hydrophobic" in fragment_name:
                    interpretation = "Destabilizes hydration (positive ΔG contribution)"
                else:
                    interpretation = "Affects hydration thermodynamics"
    ...
    def _generate_human_explanation(self, fragments: Dict[str, float], prediction: float) -> str:
        """Generate human-readable explanation for FreeSolv"""
        # Identify stabilizing vs destabilizing groups
        stabilizing = ['H-Bond Donor: -OH', 'H-Bond Donor: -NH2', 'H-Bond Donor: -NH',
                      'H-Bond Acceptor: C=O', 'H-Bond Acceptor: -O-', 'H-Bond Acceptor: -N-',
                      'Charged: Carboxylate', 'Charged: Ammonium']
        destabilizing = ['Hydrophobic: Aromatic', 'Hydrophobic: Long Alkyl', 
                        'Hydrophobic: Branched Alkyl', 'Hydrophobic: Halogens']
        
        stable_score = sum(abs(fragments.get(frag, 0)) for frag in stabilizing)
        unstable_score = sum(abs(fragments.get(frag, 0)) for frag in destabilizing)
        
        explanation = "To see how 'happy' this molecule is in water, we compared it to its 'boring twin' (carbon-only version).\n\n"
        
        if stable_score > unstable_score:
            explanation += "🔵 The BLUE parts (water-stabilizing groups) dominate this molecule!\n"
            key_groups = [f.split(': ')[1] if ': ' in f else f for f in stabilizing if abs(fragments.get(f, 0)) > 0.01]
            if key_groups:
                explanation += f"Key groups: {', '.join(key_groups)}\n"
            explanation += "These polar groups form strong hydrogen bonds and electrostatic interactions with water.\n"
            explanation += f"Prediction: {prediction:.3f} kcal/mol - FAVORABLE hydration (water-loving)"
        else:
            explanation += "🔴 The RED parts (water-destabilizing groups) dominate this molecule!\n" 
            key_groups = [f.split(': ')[1] if ': ' in f else f for f in destabilizing if abs(fragments.get(f, 0)) > 0.01]
            if key_groups:
                explanation += f"Key groups: {', '.join(key_groups)}\n"
            explanation += "These hydrophobic groups disrupt water's hydrogen bond network, requiring energy.\n"
            explanation += f"Prediction: {prediction:.3f} kcal/mol - UNFAVORABLE hydration (water-hating)"
        
        return explanation

def save_molecule_visualization(smiles: str, attributions: List[float], 
                               prediction: float, output_dir: Path, logger):
    """Create and save molecule visualization with attributions for FreeSolv"""
    try:
        ...
        # Color scheme: Blue (stabilizing) to Red (destabilizing)
        atom_colors = {}
        atom_radii = {}
        ...
        for i in range(min(len(attributions), mol.GetNumAtoms())):
            ...
            if norm_attr > 0:
                # Blue for positive (stabilizing hydration)
                intensity = abs(norm_attr)
                atom_colors[i] = (1-intensity, 1-intensity, 1.0)
            else:
                # Red for negative (destabilizing hydration)
                intensity = abs(norm_attr)
        ...
        img_path = output_dir / f"{safe_smiles}_freesolv_attribution_map.png"
...
def create_attribution_plots(explanation_data: Dict[str, Any], output_dir: Path, logger):
    """Create attribution analysis plots for FreeSolv"""
    try:
        ...
        fig.suptitle(f"MB-EIG Attribution Analysis (FreeSolv)\nSMILES: {smiles}", fontsize=14)
        ...
        axes[0, 1].set_title('MB-EIG Attributions (Blue=Stabilizing, Red=Destabilizing)')
        ...
        if fragments:
            ...
            axes[1, 0].set_title('Fragment-Level Analysis (Hydration Energy)')
            axes[1, 0].set_xticks(range(len(frag_names)))
            axes[1, 0].set_xticklabels([name.split(': ')[0] for name in frag_names], rotation=45, ha='right')
        ...
        # Save plot
        plot_path = output_dir / f"{safe_smiles}_freesolv_attribution_analysis.png"
...
def save_text_explanation(explanation_data: Dict[str, Any], output_dir: Path, logger):
    """Save human-readable text explanation for FreeSolv"""
    try:
        smiles = explanation_data['smiles']
        safe_smiles = smiles.replace('/', '_').replace('\\', '_').replace(':', '_')
        
        text_path = output_dir / f"{safe_smiles}_freesolv_explanation.txt"
        
        with open(text_path, 'w', encoding='utf-8') as f:
            f.write("MOLECULAR HYDRATION FREE ENERGY PREDICTION EXPLANATION\n")
            f.write("=" * 60 + "\n\n")
            ...
            f.write("THERMODYNAMIC INTERPRETATION:\n")
            f.write("-" * 35 + "\n")
            pred_val = explanation_data['predicted_value']
            if pred_val < 0:
                f.write(f"ΔG = {pred_val:.3f} kcal/mol (NEGATIVE)\n")
                f.write("→ Hydration is thermodynamically FAVORABLE\n")
                f.write("→ Molecule prefers aqueous environment\n")
                f.write("→ Strong water interactions compensate for cavity formation\n\n")
            else:
                f.write(f"ΔG = {pred_val:.3f} kcal/mol (POSITIVE)\n")  
                f.write("→ Hydration is thermodynamically UNFAVORABLE\n")
                f.write("→ Molecule prefers non-polar environment\n")
                f.write("→ Cavity formation cost exceeds water interaction benefits\n\n")
            ...
            f.write("KEY MOLECULAR FRAGMENTS:\n")
            f.write("-" * 30 + "\n")
            fragments = explanation_data['fragments']
            if fragments:
            ...
            f.write("Generated by MB-EIG Explainer for FreeSolv\n")
...
def predict_single_molecule(smiles: str, output_dir: Path = None, model_path: Path = None):
    ...
    logger.info("=" * 80)
    logger.info("FREESOLV SINGLE MOLECULE PREDICTION WITH MB-EIG")
    logger.info("=" * 80)
    ...
    try:
        ...
        if model_path is None:
            model_path = MODELS_DIR / 'best_graphormer_freesolv.pth'
        ...
        json_path = output_dir / f"{safe_smiles}_freesolv_prediction.json"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="FreeSolv Single Molecule Prediction with MB-EIG")
    ...
    parser.add_argument('--model', type=str, default=None,
                       help='Path to trained model (default: ../models/best_graphormer_freesolv.pth)')
    ...
    if result:
        ...
        print(f"Predicted Hydration Free Energy: {result['predicted_value']:.3f} {result['unit']}")
        
        # Thermodynamic interpretation
        if result['predicted_value'] < 0:
            print(f"Thermodynamics: FAVORABLE hydration (ΔG < 0)")
        else:
            print(f"Thermodynamics: UNFAVORABLE hydration (ΔG > 0)")
